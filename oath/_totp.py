...
def totp(key, format='dec6', period=30, t=None, t0=0, hash=hashlib.sha1):
    '''
    ...
    if isinstance(t0, datetime.datetime):
        t0 = calendar.timegm(t0.utctimetuple())
    elif isinstance(t0, int) is False:
        raise ValueError("t0 must be an integer or a datetime.datetime")
        
    T = int((t-t0)/period)
    return hotp(key, T, format=format, hash=hash)

def accept_totp(key, response, format='dec6', period=30, t=None, t0=0,
    ...
    for i in range(max(-divmod(t, period)[0],-backward_drift),forward_drift+1):
        d = (drift+i) * period
        if _utils.compare_digest(totp(key, format=format, period=period, hash=hash, t=t+d, t0=t0), response):
            return True, drift+i
    return False, 0
